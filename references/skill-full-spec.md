...
- 固定输出目录：`<项目目录>/<年份>年软件著作权申请资料/<软件全称>/`。任务的 `analysis/`、`草稿/`、`正式资料/`、`截图/` 和 `用户截图/` 直接位于该目录下。
- 只有测试 skill 自身时才允许显式指定临时目录；面向用户生成材料时必须写入任务目录，不得散落到工作区根目录。
...
`application_purpose`、`main_functions`、`technical_characteristics` 三个字段需特别注意：

- **软件的主要功能**（`main_functions`）：2026 年新政要求 500-1300 字。结构为「技术架构概述 → 功能模块逐项详述」：先交代系统的技术架构（B/S 或 C/S、前后端框架、数据库、通信方式），再逐模块展开功能描述，每个模块写清它做什么、覆盖哪些具体场景。不要单独写研发背景段和应用场景段——这些信息已在 `开发目的` 和 `面向领域/行业` 字段中覆盖。
- **软件的技术特点**（`technical_characteristics`）：格式为「预定义分类, 描述」。分类从官方列表中选择（APP / 信息安全软件 / 游戏软件 / 大数据软件 / 教育软件 / 人工智能软件 / 金融软件 / VR软件 / 医疗软件 / 5G软件 / 地理信息软件 / 小程序 / 云计算软件 / 物联网软件 / 智慧城市软件 / 其他），描述控制在 100 字以内，写系统核心的技术亮点。分类前缀不可省略——缺失时 `generate_application_info.py` 输出 `FIELD_WARNING`。

业务理解阶段就必须输出足够丰满的文本，不得在申请表生成时再用模板扩写。
...
根据分析结果，按以下分组向用户确认字段。结构化字段数据（顺序、枚举值、字数限制）见 `references/application_fields.json`，字段口径说明见 `references/application_fields.md`。

**参考素材 0+3 拆分（方案 v2，详见 `references/reference_materials.md`）**：主体法定字段（著作权人、证件信息）优先查同一工作区下的主数据表；项目技术字段（硬件环境、编程语言、源程序量）来自代码/部署/用户确认推导，不从「最近一份」直接复制；结构差异化用全批画像避让。
...
- 经办人身份证号码
- 经办人职务
- AI开发限制声明（留空不自动填，由用户人工决定是否填写及内容并签署；skill 不代填）

项目可推断字段可以先给建议值；硬件/系统环境必须允许用户选择建议值或手动填写。

**环境字段预设复用（优先）**：在逐项询问硬件/系统环境字段前，先检查 `references/env_presets.json`。若 `presets` 数组非空，列出所有预设的名称和内容摘要，询问用户"是否复用其中一组预设？"——用户可选某一组直接填入、选"不用，手动填写"、或选"新建一组预设"（填入当前项目值后追加到 `presets` 数组）。若 `presets` 为空或用户选择手动填写，按以下字段口径逐项确认。
...
### 6b. 材料证据计划（新任务默认 v2 路径）

> 新任务必须使用 v2 材料证据计划；legacy v1 只兼容已经存在且明确不返工的旧任务。多仓库、版本锁定或补正重提场景尤其不得降级。
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/code_boundary_report.py \
  --workspace "<年份目录>/<年份>年软件著作权申请资料"
```

按独立软件申请：每项任务生成 `草稿/软件边界说明.md` 备案（代码来源、独有/共享范围），并输出整批两两共享率报告。只记录备案，不裁定是否合并。
...
操作手册位于 `草稿/`，图片位于同级的 `截图/`，因此必须按标准 Markdown 相对路径写成 `![图表名称](../截图/文件名.png)`。禁止写成 `截图/文件名.png`：该路径实际指向不存在的 `草稿/截图/`，在 Obsidian 等会按文件名模糊解析的工具中，可能误加载其他软著任务的同名图片。
...
python3 ${CLAUDE_SKILL_DIR}/scripts/content_quality_check.py --manual 草稿/操作手册.md
```
该命令会执行 22 项自动门禁检查——技术术语、章节编号、表格密度、功能清单、截图覆盖、AI套话、角色贯穿、多端覆盖、简介-功能清单对等、表格信息增量、术语一致性、截图位置、代码泄漏、数据字典/内部标识符泄漏等。门禁配置和禁止术语词表见 `references/quality_gates.json`。0 errors 才能进入下一步。
...
- 申请表是否包含 AI 开发限制声明（留空待用户人工填写）、经办人姓名、身份证号码、职务等 2026 新政新增字段
- 代码材料是否只来自用户确认的完整文件
...
7. 整批复检（方案 v2 决策④）：提交前运行 batch_risk_report.py 对全批（含正式 DOCX）出风险报告——确定性硬错误必须修复；相似度高风险人工复核、中低风险放行；旧批次按决策③冻结不返工。
...
python3 ${CLAUDE_SKILL_DIR}/scripts/batch_risk_report.py --workspace "<年份目录>/<年份>年软件著作权申请资料"
```
