...
| --- | --- | --- |
| 触发方式 | 未生成计划文件即走 v1（脚本自动检测） | 存在 `草稿/材料证据计划.json` 且 schema_version==3 时自动走 v2 |
...
1. **默认 v1**：单一明显项目、证据路径清晰时。
2. **必须 v2**：满足任一——a) 项目跨多个仓库且操作手册覆盖多端；b) 用户要求可追溯的版本锁定；c) 补正重提需证明代码代表性。
