...
Gate profiles and forbidden terms are loaded from references/quality_gates.json.
"""
...
def _load_gate_config() -> dict:
    config_path = Path(__file__).resolve().parents[1] / "references" / "quality_gates.json"
    with open(config_path, encoding="utf-8") as f:
        return json.load(f)

GATE_PROFILES: list[dict] = _load_gate_config()["gate_profiles"]
FORBIDDEN_TERMS: list[tuple[str, str, str]] = [
    tuple(item) for item in _load_gate_config()["forbidden_terms"]
]
